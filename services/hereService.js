...
module.exports = function (context, space, inboundrequest, request) {
    ...
    for (var i = 0; i < roster.length; i++) {
        result +=  "<li>"+context.spacemap[space][i].name + " present " + context.timeSince(context.spacemap[space][i].time) + " ago</li>";
    }
    ...
    for (var i = 0; i < roster.length; i++) {
        result +=  "<li>"+context.lastspacemap[space][i].name + " left " + context.timeSince(context.lastspacemap[space][i].time) + " ago</li>";
    }
    return result + "</ul>";
};
